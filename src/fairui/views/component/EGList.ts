module fairui {
	...
	export class EGList extends egret.EventDispatcher {
		...
		/**向左按钮 */
		private _btn_left: EButton | fairygui.GButton;
		/**向右按钮 */
		private _btn_right: EButton | fairygui.GButton;
        ...
        */
		...
		private listItemRender(index: number, obj: fairygui.GObject): void {
			...
			//列表渲染单个条目
			let evt: EGListEvent = new EGListEvent(EGListEvent.RENDER);
			evt.index = index;
			evt.obj = obj;
			...
			if (index == (this._array.length - 1)) {
				...
				let completeEvt: EGListEvent = new EGListEvent(EGListEvent.COMPLETE);
