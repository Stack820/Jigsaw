module fairui {
	import EventObj = com.events.EventObj;
	...
	export class BaseSprite extends fairygui.GComponent {
		...
		*/
		...
		/**显示引导:通过name找到目标 */
		public showGuide(panelId: number, name: string = '', parent: fairygui.GComponent = null, fixX: number = 0, fixY: number = 0, direction: number = 1): void {
			if (parent == null) {
				parent = this;
			}
		...
		/**显示引导:通过tagart找到目标 */
		public showGuideTagart(panelId: number, tagart: fairygui.GObject, name: string = '', fixX: number = 0, fixY: number = 0, direction: number = 1, showTalk: boolean = true, guideTalkExtendMap: string = "",isAlertLayer:boolean=false): void {

		}

		/**显示引导:通过index找到目标 */
		public showGuideTagartIndex(panelId: number, parent: fairygui.GComponent = null, index: number = 0, fixX: number = 0, fixY: number = 0, direction: number = 1): void {
			if (parent == null) {
				parent = this;
			}
		}

		/**强制显示指定的引导:不判断panelId */
		public showDesignatedGuide(name: string = '', parent: fairygui.GComponent = null, index: number = 0, fixX: number = 0, fixY: number = 0, direction: number = 1) {
			let objTemp: fairygui.GObject = null;
			if (name != null && name.length > 0) {
				objTemp = parent[name];
			} else {
				objTemp = parent.getChildAt(index);
			}
			if (objTemp) {
				this.addGuide(objTemp, fixX, fixY, direction);
			}
		}

		/**添加引导,并且设置位置 */
		public addGuide(tagart: fairygui.GObject, fixX: number = 0, fixY: number = 0, direction: number = 1, fixTalkDirection: number = -1, showTalk: boolean = true, guideTalkExtendMap: string = "",isAlertLayer:boolean=false): void {
			let point: egret.Point = tagart.localToGlobal(0, 0);
		}

		/**隐藏(关闭)引导 */
		public hideGuide(panelId: number = 0, name: string = ''): void {
			if( Config.isGuide ){
			}			
		}
