module fairui {
    ...
    export class EButton extends BaseButton {
        ...
        public set notice(value: boolean) {
            this.dot.x = this._iconObject.x + this._iconObject.width - this.dot.width;
            this.dot.y = this._iconObject.y;

            this.org_dot_x = this.dot.x;

            egret.superSetter(EButton, this, "notice", value);

            if (value) { //设置随机跳动的小红dian
                this.swing(true);
            } else {
                this.swing(false);
            }
        ...
        public btnUrl(val: string, isThreeStateBtn: boolean = true): void {
            ...
            if (val.indexOf("ui:") != -1) {
                ...
                if (this.icon != val && this.icon.indexOf("ui:") == -1) {
