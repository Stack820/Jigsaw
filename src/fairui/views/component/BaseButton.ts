module fairui {
    ...
    export class BaseButton extends fairygui.GButton {
        ...
        public set noticeType(value: Array<number>) {
            ...
            if( !EventManager.hasEventListener( EventType.ShowOrHideButtonDot , flash.bind( this.showOrHideDot , this ) , this ) ){
                EventManager.addEventListener( EventType.ShowOrHideButtonDot, flash.bind( this.showOrHideDot , this ), this);
            }            
        }
        ...
        /**根据Type显示小红点状态 */
        public showOrHideDot(type: number, value: boolean): void {
            if (this._noticeType.indexOf(type) == -1) {
                return;
            }
            this._noticeState.setItem(type, value);
            let isCanNotice: boolean = false;
            this._noticeState.forEach((key: any, info: boolean) => {
                if (info) {
                    isCanNotice = info;
                }
            }, this);
            this.notice = isCanNotice;
        }
        ...
        /**设置显示提示 */
        public set notice(value: boolean) {
            if (this.dot) {
                if (!this.visible) {
                    this.dot.visible = false;//按钮本身没有显示,不显示小红点
                } else {
        ...
        public dispose(): void {
            EventManager.removeEventListener(com.events.EventType.ShowOrHideButtonDot, this.showOrHideDot, this);
